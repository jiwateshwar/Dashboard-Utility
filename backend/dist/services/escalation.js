...
async function notify(userId, dashboardId, entityType, entityId, ruleName, message) {
    const id = uuid();
    await query(`INSERT INTO escalations (id, dashboard_id, entity_type, entity_id, rule_name, message, notified_user_id)
     VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, dashboardId, entityType, entityId, ruleName, message, userId]);
    await query(`INSERT INTO notifications (id, user_id, message)
     VALUES ($1, $2, $3)`, [uuid(), userId, message]);
}
async function loadRules(dashboardId) {
    const { rows } = await query(`SELECT * FROM escalation_rules WHERE dashboard_id = $1 AND is_active = true`, [dashboardId]);
    return rows;
}
async function runDefaultRules(dash) {
    const tasks = await query(`SELECT t.id, t.owner_id, t.created_at
     FROM tasks t
     WHERE t.dashboard_id = $1 AND t.is_archived = false AND t.status != 'Closed Accepted'`, [dash.id]);
    ...
    const risks = await query(`SELECT id, impact_level FROM risks WHERE dashboard_id = $1 AND is_archived = false`, [dash.id]);
    ...
    const decisions = await query(`SELECT id, decision_deadline FROM decisions WHERE dashboard_id = $1 AND status = 'Pending' AND is_archived = false`, [dash.id]);
...
async function runRule(dash, rule) {
...
export async function runEscalations() {
    const dashboards = await query(`SELECT id, primary_owner_id FROM dashboards WHERE is_active = true`);
    for (const dash of dashboards.rows) {
        const rules = await loadRules(dash.id);
        if (rules.length === 0) {
            await runDefaultRules(dash);
            continue;
        }
        for (const rule of rules) {
            await runRule(dash, rule);
        }
    }
}
