...
router.post("/", async (req, res) => {
    ...
    const { account_name, account_type, region } = req.body;
    if (!account_name)
        return res.status(400).json({ error: "account_name required" });
    const id = uuid();
    await query(`INSERT INTO accounts (id, account_name, account_type, region)
     VALUES ($1, $2, $3, $4)`, [id, account_name, account_type || null, region || null]);
    res.json({ id });
});
router.patch("/:id", async (req, res) => {
    const { id } = req.params;
    const { dashboard_id, account_name, account_type, region, is_active } = req.body;
    const ok = await canManageAccounts(req.session.userId, dashboard_id);
    if (!ok)
        return res.status(403).json({ error: "Not allowed" });
    await query(`UPDATE accounts
     SET account_name = COALESCE($2, account_name),
         ...
         updated_at = now()
     WHERE id = $1`, [id, account_name || null, account_type || null, region || null, is_active]);
    res.json({ ok: true });
});
