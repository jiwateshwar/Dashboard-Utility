...
router.get("/", async (req, res) => {
    ...
    const { rows } = await query(`SELECT t.*, u.name as owner_name,
...
router.post("/", async (req, res) => {
    const userId = req.session.userId;
    const { dashboard_id, category_id, account_id, item_details, owner_id, target_date, sla_days, rag_status, publish_flag } = req.body;
    ...
    const access = await query(`SELECT 1 FROM dashboard_access WHERE dashboard_id = $1 AND user_id = $2 AND can_view = true`, [dashboard_id, owner_id]);
    ...
    await query(`INSERT INTO tasks
     ...
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'Open', $10, $11)`, [id, dashboard_id, category_id, account_id, item_details, owner_id, userId, target_date, sla_days || null, rag_status || "Green", publish_flag ?? false]);
    res.json({ id });
});
router.patch("/:id", async (req, res) => {
    const { id } = req.params;
    const userId = req.session.userId;
    const { item_details, owner_id, target_date, rag_status, publish_flag, status } = req.body;
    const task = await query(`SELECT dashboard_id, owner_id, created_by, status FROM tasks WHERE id = $1`, [id]);
    if (task.rows.length === 0)
        return res.status(404).json({ error: "Not found" });
    const dashboardId = task.rows[0].dashboard_id;
    const canEdit = await canEditDashboard(userId, dashboardId);
    if (!canEdit)
        return res.status(403).json({ error: "No edit access" });
    if (owner_id) {
        const access = await query(`SELECT 1 FROM dashboard_access WHERE dashboard_id = $1 AND user_id = $2 AND can_view = true`, [dashboardId, owner_id]);
    ...
    await query(`UPDATE tasks
     SET item_details = COALESCE($2, item_details),
         ...
         updated_at = now()
     WHERE id = $1`, [id, item_details || null, owner_id || null, target_date || null, rag_status || null, publish_flag, status || null]);
...
router.post("/:id/approve", async (req, res) => {
    const { id } = req.params;
    const userId = req.session.userId;
    const task = await query(`SELECT dashboard_id, created_by FROM tasks WHERE id = $1`, [id]);
    if (task.rows.length === 0)
        return res.status(404).json({ error: "Not found" });
    const dashboardId = task.rows[0].dashboard_id;
    const creatorId = task.rows[0].created_by;
