...
router.get("/", async (req, res) => {
    ...
    const { rows } = await query(`SELECT DISTINCT d.*
     FROM dashboards d
     LEFT JOIN dashboard_access da ON da.dashboard_id = d.id
     WHERE d.primary_owner_id = $1 OR d.secondary_owner_id = $1 OR da.user_id = $1`, [userId]);
    res.json(rows);
});
router.post("/", async (req, res) => {
    ...
    const { name, description, primary_owner_id, secondary_owner_id } = req.body;
    if (!name || !primary_owner_id) {
        return res.status(400).json({ error: "Missing fields" });
    }
    const id = uuid();
    await query(`INSERT INTO dashboards (id, name, description, primary_owner_id, secondary_owner_id)
     VALUES ($1, $2, $3, $4, $5)`, [id, name, description || null, primary_owner_id, secondary_owner_id || null]);
    res.json({ id });
});
router.patch("/:id", async (req, res) => {
    ...
    const { name, description, primary_owner_id, secondary_owner_id, is_active } = req.body;
    await query(`UPDATE dashboards
     SET name = COALESCE($2, name),
         ...
         updated_at = now()
     WHERE id = $1`, [id, name || null, description || null, primary_owner_id || null, secondary_owner_id || null, is_active]);
    res.json({ ok: true });
});
router.get("/:id/summary", async (req, res) => {
    ...
    const tasks = await query(`SELECT status, rag_status, created_at FROM tasks WHERE dashboard_id = $1 AND is_archived = false`, [id]);
    const risks = await query(`SELECT status, impact_level, probability, target_mitigation_date FROM risks WHERE dashboard_id = $1 AND is_archived = false`, [id]);
    const decisions = await query(`SELECT status, decision_deadline FROM decisions WHERE dashboard_id = $1 AND is_archived = false`, [id]);
    const taskStats = {
...
router.post("/:id/access", async (req, res) => {
    ...
    const { target_user_id, can_view, can_edit } = req.body;
    if (!target_user_id)
        return res.status(400).json({ error: "Missing target_user_id" });
    await query(`INSERT INTO dashboard_access (dashboard_id, user_id, can_view, can_edit)
     VALUES ($1, $2, $3, $4)
