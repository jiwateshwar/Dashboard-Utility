...
router.get("/", async (req, res) => {
    ...
    const { rows } = await query(`SELECT * FROM publishing_snapshots WHERE dashboard_id = $1 ORDER BY cycle_date DESC`, [dashboard_id]);
    res.json(rows);
});
router.post("/generate", async (req, res) => {
    ...
    await query(`INSERT INTO publishing_snapshots (id, dashboard_id, cycle_date, content_json)
     VALUES ($1, $2, $3, $4)`, [id, dashboard_id, today, JSON.stringify(content)]);
    res.json({ id });
});
router.get("/email/:dashboardId", async (req, res) => {
    ...
    const { rows } = await query(`SELECT content_json, cycle_date FROM publishing_snapshots
     WHERE dashboard_id = $1 ORDER BY cycle_date DESC LIMIT 1`, [dashboardId]);
