...
router.post("/login", async (req, res) => {
    ...
    const { rows } = await query(`SELECT id, is_active FROM users WHERE email = $1`, [email]);
    if (rows.length === 0) {
...
router.get("/me", requireAuth, async (req, res) => {
    const { rows } = await query(`SELECT id, name, email, manager_id, level, role, is_active FROM users WHERE id = $1`, [req.session.userId]);
    res.json(rows[0]);
});
