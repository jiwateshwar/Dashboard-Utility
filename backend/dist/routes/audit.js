...
router.get("/", async (req, res) => {
    const { entity_type, entity_id, dashboard_id } = req.query;
    const userId = req.session.userId;
    const role = await getUserRole(userId);
    if (role !== "Admin") {
        if (!dashboard_id)
            return res.status(403).json({ error: "Dashboard scope required" });
        const owner = await isDashboardOwner(userId, dashboard_id);
        if (!owner)
            return res.status(403).json({ error: "Owners only" });
    }
    const { rows } = await query(`SELECT a.*
     FROM audit_log a
     ...
     LIMIT 500`, [entity_type || null, entity_id || null, dashboard_id || null]);
    res.json(rows);
});
