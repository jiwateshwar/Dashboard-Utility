...
router.get("/", async (req, res) => {
  ...
  const { rows } = await query(
    `SELECT * FROM categories WHERE dashboard_id = $1 ORDER BY name`,
    [dashboard_id]
  );
  res.json(rows);
});
...
router.post("/", async (req, res) => {
  const { dashboard_id, name } = req.body as any;
  if (!dashboard_id || !name) return res.status(400).json({ error: "Missing fields" });
  const owner = await isDashboardOwner(req.session.userId!, dashboard_id);
  if (!owner) return res.status(403).json({ error: "Owners only" });
  const id = uuid();
  await query(
    `INSERT INTO categories (id, dashboard_id, name) VALUES ($1, $2, $3)`,
...
router.patch("/:id", async (req, res) => {
  const { id } = req.params;
  const { dashboard_id, name, is_active } = req.body as any;
  if (!dashboard_id) return res.status(400).json({ error: "dashboard_id required" });
  const owner = await isDashboardOwner(req.session.userId!, dashboard_id);
  if (!owner) return res.status(403).json({ error: "Owners only" });
  await query(
